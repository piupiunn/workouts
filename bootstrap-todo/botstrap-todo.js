...
const firstCardBody = document.querySelectorAll(".card-body")[0]; //Form ekleme işlemi başarılı yada başarız olduğunda todo kartının iç kısmına ve hemen altına alert eklemek için kartı parent olarak seçmemiz gerekli.
...
function clearAllTodos() {
  if (confirm("Tüm todoları silmek istediğinize emin misiniz?")) {
    ...
    while (todoList.firstElementChild != null) {
      // todo listin içinde hiç bir çocuğu olmazsa null yani boş bir değer döndürür. Bizde "todolist null değilse ilk childını sil" diye bir döngü oluşturduk. todolistin için null yani boş değer olaana kadar ilk çocuğu silinecek ve sonunda tüm çocukları silindiğinde null değeri dönecek ve döngümüz duracak.
...
function deleteTodo(e) {
  if (e.target.className === "fa fa-remove") {
    ...
    deleteFromStorage(e.target.parentElement.parentElement.textContent); //deleteFromStorage fonksiyonuna silinmesi için tıklanan <li> elementinin içeriğini yolluyoruz.
...
function addTodoToUI(newTodo) {
  ...
  listItem.className = "list-group-item d-flex justify-content-between"; //Li ye class ismini verdik.
